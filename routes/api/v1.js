...
router.route("/filestorage").get((req, res) => {
    ...
    FileStorage.findOne({ owner: req.user.id }).then(storage => {
        res.json({ success: true, storage: storage });
    }).catch(err => {
    ...
    FileStorage.exists({ owner: req.user.id }).then(exists => {
        if (exists) return res.json({ success: false, message: req.t("api.filestorage.exists") });
        ...
        storage.save().then(storage => {
            res.json({ success: true, message: req.t("api.filestorage.success") });
        }).catch(err => {
            res.status(500).json({ success: false, message: err.toString() });
        });
    }).catch(err => {
...
router.route("/filestorage/folder").post((req, res) => {
    ...
    if (!folderName) return res.json({ success: false, message: req.t("api.filestorage.folder.namemissing") });
    if (!folderPath) return res.json({ success: false, message: req.t("api.filestorage.pathmissing") });
    if (!utils.foldernameValid(folderName)) return res.json({ success: false, message: req.t("api.filestorage.folder.nameinvalid") });
    
    FileStorage.findOne({ owner: req.user.id }).then(storage => {
        if (!storage) return res.json({ success: false, message: req.t("api.filestorage.missing") });
        if (folderPath != "/") {
            if (!storage.folders.find(folder => folder.path + `${folder.name}/` == folderPath)) return res.json({ success: false, message: req.t("api.filestorage.pathinvalid") });
        }
        if (storage.folders.find(folder => folder.name == folderName && folder.path == folderPath) || storage.files.find(file => file.name == folderName && file.path == folderPath)) return res.json({ success: false, message: req.t("api.filestorage.fexists", { "0": folderName }) });
        ...
        storage.save().then(storage => {
            res.json({ success: true, message: req.t("api.filestorage.folder.created", { "0": folderName }), folder: storage.folders.find(f => f.name == folderName && f.path == folderPath) });
        }).catch(err => {
            res.status(500).json({ success: false, message: err.toString() });
        });
    }).catch(err => {
    ...
    if (!folderId) return res.json({ success: false, message: req.t("api.filestorage.folder.idmissing") });
    if (folderName && !utils.foldernameValid(folderName)) return res.json({ success: false, message: req.t("api.filestorage.folder.nameinvalid") });

    FileStorage.findOne({ owner: req.user.id }).then(storage => {
        if (!storage) return res.json({ success: false, message: req.t("api.filestorage.missing") });
        if (!storage.folders.find(f => f.id == folderId)) return res.json({ success: false, message: req.t("api.filestorage.folder.idinvalid") });

        let index = storage.folders.findIndex(f => f.id == folderId);
        let changed = false;
        if (folderPath && folderPath != "/" && !storage.folders.find(folder => folder.path + `${folder.name}/` == folderPath)) return res.json({ success: false, message: req.t("api.filestorage.pathinvalid") });
        ...
        if (folderName != storage.folders[index].name || folderPath != storage.folders[index].path) {
            if (storage.folders.find(f => f.name == folderName && f.path == folderPath) || storage.files.find(f => f.name == folderName && f.path == folderPath)) return res.json({ success: false, message: req.t("api.filestorage.fexists", { "0": folderName }) });
        ...
        if (!changed) return res.json({ success: false, message: req.t("api.filestorage.folder.unchanged") });

        storage.save().then(storage => {
            res.json({ success: true, message: req.t("api.filestorage.folder.changed", { "0": folderName }), folder: storage.folders.find(f => f.id == folderId) });
        }).catch(err => {
            res.status(500).json({ success: false, message: err.toString() });
        });
    }).catch(err => {
    ...
    FileStorage.findOne({ owner: req.user.id }).then(storage => {
        ...
        let index = storage.folders.findIndex(f => f.id == folderId);
        if (index == -1) return res.json({ success: false, message: req.t("api.filestorage.folder.idinvalid") });
        ...
        storage.save().then(storage => {
            res.json({ success: true, message: req.t("api.filestorage.folder.deleted", { "0": folderName }) });
        }).catch(err => {
            res.status(500).json({ success: false, message: err.toString() });
        });
    }).catch(err => {
...
router.route("/filestorage/file").post(fileUpload({ useTempFiles: true, tempFileDir: path.join(process.cwd(), "tmp") }), (req, res) => {
    ...
    if (!filePath) return res.json({ success: false, message: req.t("api.filestorage.pathmissing") });
    if (!req.files) return res.json({ success: false, message: req.t("api.filestorage.filemissing") });

    FileStorage.findOne({ owner: req.user.id }).then(storage => {
        if (!storage) return res.json({ success: false, message: req.t("api.filestorage.missing") });
        if (filePath != "/") {
            if (!storage.folders.find(folder => folder.path + `${folder.name}/` == filePath)) return res.json({ success: false, message: req.t("api.filestorage.pathinvalid") });
        }
        ...
        if (req.files.file) {
            ...
            promise.then(result => {
                storage.save().then(storage => {
                    res.json({ success: true, message: req.t("api.filestorage.file.uploaded", { "0": file.name }), file: storage.files.find(f => f.name == result.name && f.path == result.path) });
                }).catch(err => {
                    res.status(500).json({ success: false, message: err.toString() });
                });
            }).catch(err => {
        ...
        // else if there's multiple files to upload
            ...
            if (files.length == 0) return res.json({ success: false, message: req.t("api.filestorage.files.skipped") });

            // Check if files are too big
            files.forEach(file => {
                size += file.size;
            });
            if (storage.size + size > storage.maxSize) return res.json({ success: false, message: req.t("api.filestorage.files.toobig"), size: size, available: storage.maxSize - storage.size });
    ...
    FileStorage.findOne({ owner: req.user.id }).then(storage => {
        if (!storage) return res.json({ success: false, message: req.t("api.filestorage.missing") });
        if (!storage.files.find(f => f.id == fileId)) return res.json({ success: false, message: req.t("api.filestorage.file.idinvalid") });

        let index = storage.files.findIndex(f => f.id == fileId);
        let changed = false;
        if (fileName && !utils.filenameValid(fileName)) return res.json({ success: false, message: req.t("api.filestorage.file.nameinvalid") });
        if (filePath && filePath != "/" && !storage.folders.find(folder => folder.path + `${folder.name}/` == filePath)) return res.json({ success: false, message: req.t("api.filestorage.pathinvalid") });
        ...
        if (fileName != storage.files[index].name || filePath != storage.files[index].path) {
            if (storage.files.find(f => f.name == fileName && f.path == filePath) || storage.folders.find(f => f.name == fileName && f.path == filePath)) return res.json({ success: false, message: req.t("api.filestorage.fexists", { "0": fileName }) });1
        ...
        if (!changed) return res.json({ success: false, message: req.t("api.filestorage.file.unchanged") });

        storage.save().then(storage => {
            res.json({ success: true, message: req.t("api.filestorage.file.changed", { "0": fileName }), file: storage.files.find(f => f.id == fileId) });
        }).catch(err => {
            res.status(500).json({ success: false, message: err.toString() });
        });
    }).catch(err => {
    ...
    FileStorage.findOne({ owner: req.user.id }).then(storage => {
        ...
        storage.size -= file.size;
        storage.save().then(storage => {
            res.json({ success: true, message: req.t("api.filestorage.file.deleted", { "0": file.name }) });
        }).catch(err => {
            res.status(500).json({ success: false, message: err.toString() });
        });
    }).catch(err => {
