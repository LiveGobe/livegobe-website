...
async function executeWikiModule(options = {}, moduleName, functionName, args = []) {
  ...
  if (!options.moduleCache) {
    options.moduleCache = new Map();  // <--- per-request cache
  }
  ...
  // Check cache
  if (options.moduleCache.has(normalized)) {
    const cached = options.moduleCache.get(normalized);

    // If this is a plain export (like return {...})
    if (functionName === "__default__") {
      return { __exports__: cached };
    }

    // Return requested function if it exists
    if (typeof cached[functionName] === "function") {
      return await cached[functionName](...(args || []));
    }

    return cached; // string or scalar
  }
  ...
  const sandbox = {
    ...
    require: async function (name) {
      try {
        ...
        // Use the SAME options so moduleCache stays shared
...
`;
    ...
    options.moduleCache.set(normalized, exported);
